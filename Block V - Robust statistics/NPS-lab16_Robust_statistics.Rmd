...
Following the definitions outlined during the very first class of the module, explicit robust estimators are readily available in R. There is not very much to say about it here, let us only try to recreate the table presented in class.
...
What we would like to to is to code the iterative algorithm described in slide  13, but with an additional (robust) scale estimate $\hat{\sigma}$.
...
The so created function can be directly applied to the toy univariate data we have seen so far:

```{r}
manual_M_location(x = x,k = 1.5,type = "Huber")
manual_M_location(x = x,k = 4.68,type = "Tukey")
```
...
The `fit_MCD` object contains the final (raw and reweighted) estimates of location and scatter, the resulting robust Mahalanobis distances and the best subset used for computing the raw estimates. 
...
$$
($q_{p, \alpha}$ is the $\alpha$ level quantile of a $\chi^2_p$ distribution)
and a finite sample correction factor, to make it consistent at the normal model and unbiased at small samples.
...
Notice that by default, unless specifying`raw.only=TRUE`, covMcd performs a reweighting step for improving the efficiency of the final estimator. In details, the new weigths are computed as follows:

$$
W\left(d^{2}\right)=I\left(d^{2} \leqslant \chi_{p, 0.975}^{2}\right)
$$
where $d^2$ is the squared Mahalanobis distance based on the scaled raw MCD.
